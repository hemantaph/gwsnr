...
def spline_interp_4pts_numba(x_eval, x_pts, y_pts, condition_i):
    ...
    if condition_i == 2:
        ...
        m1 = ((y2 - y1) / (x2 - x1)) * ((x1 - x0) / (x2 - x0)) + (
            (y1 - y0) / (x1 - x0)
        ) * ((x2 - x1) / (x2 - x0))
        m2 = ((y3 - y2) / (x3 - x2)) * ((x2 - x1) / (x3 - x1)) + (
            (y2 - y1) / (x2 - x1)
        ) * ((x3 - x2) / (x3 - x1))
        ...
        return y_pts[0] + (y_pts[1] - y_pts[0]) * (x_eval - x_pts[0]) / (
            x_pts[1] - x_pts[0]
        )
    elif condition_i == 3:
        return y_pts[2] + (y_pts[3] - y_pts[2]) * (x_eval - x_pts[2]) / (
            x_pts[3] - x_pts[2]
        )
    ...
    for i in range(4):  # Loop over q-dimension index offset
        ...
        for j in range(4):  # Loop over mtot-dimension index offset
            ...
            partialsnr_along_m[j] = spline_interp_4pts_numba(
                a1_new, a1_array, partialsnr_along_a1, int_a1
            )

        partialsnr_along_q[i] = spline_interp_4pts_numba(
            mtot_new, mtot_array, partialsnr_along_m, int_m
        )

    final_snr = spline_interp_4pts_numba(q_new, q_array, partialsnr_along_q, int_q)
...
def spline_interp_4x4x4x4pts_batched_numba(
    ...
    for i in prange(n):
        ...
        m_idx, m_condition_i_batch[i] = find_index_1d_numba(
            mtot_array, mtot_new_batch[i]
        )
        ...
        cube_4x4x4x4[i] = snrpartialscaled_array[
            q_idx - 1 : q_idx + 3,
            m_idx - 1 : m_idx + 3,
            a1_idx - 1 : a1_idx + 3,
            a2_idx - 1 : a2_idx + 3,
...
def get_interpolated_snr_aligned_spins_numba(
    ...
    Mc = ((mass_1 * mass_2) ** (3.0 / 5.0)) / ((mass_1 + mass_2) ** (1.0 / 5.0))
    A1 = Mc ** (5.0 / 6.0)
...
def spline_interp_4x4pts_batched_numba(
    ...
    for i in prange(n):
        ...
        # Fill the 4D cube with the corresponding values from snrpartialscaled_array
        cube_4x4x4x4[i] = snrpartialscaled_array[
            q_idx - 1 : q_idx + 3, m_idx - 1 : m_idx + 3
        ]
