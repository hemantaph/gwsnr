...
class GWSNR:
    ...
    def __init__(
        ...
        snr_th_net=8.0,
        ann_path_dict=None,
        ann_partialscaled_approximant="IMRPhenomD",
    ):
        ...
        # ANN method still needs the partialscaledSNR interpolator.
            ...
            self.model_dict, self.scaler_dict = self.ann_initilization(ann_path_dict, detector_list, sampling_frequency, minimum_frequency, waveform_approximant, snr_th)
            # dealing with interpolator
            # Note: by default the partialscaledSNR interpolator used in ANN method corresponds to 'IMRPhenomD' waveform approximant.
            self.waveform_approximant = ann_partialscaled_approximant
            self.path_interpolator = self..interpolator_setup(
    ...
    def ann_initilization(self, ann_path_dict, detector_list, sampling_frequency, minimum_frequency, waveform_approximant, snr_th):
        ...
        for detector in detector_list:
            ...
            if not os.path.exists(ann_path_dict[detector]['model_path']):
                ...
                model_dict[detector] = load_h5_from_module('gwsnr', 'ann', ann_path_dict[detector]['model_path'])
            else:
                # load the model from the given path
                model_dict[detector] = load_h5(ann_path_dict[detector]['model_path'])
    ...
    def snr_with_ann(
        ...
        for i, det in enumerate(detectors):
            ...
            optimal_snr[det][idx_tracker] = model[det].predict(x, verbose=0).flatten()
