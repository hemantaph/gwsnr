...
def cubic_spline_4d_jax(q_array, mtot_array, a1_array, a2_array, snrpartialscaled_array, q_new, mtot_new, a1_new, a2_new):
    ...
    # Step 1: Get local patches and coordinate stencils
    ...
    # # Step 2: Interpolate along a2 (innermost axis) for all [i, j, k]
    # interp_a2 = vmap(
    #     vmap(
    #         vmap(lambda y: cubic_function_4pts_jax(a2_new, a2s, y, int_a2), in_axes=0),
    #         in_axes=0
    #     ),
    #     in_axes=0
    # )(F)  # shape: (4,4,4)

    # # Step 3: Interpolate along a1 for all [i, j]
    # interp_a1 = vmap(
    #     vmap(lambda y: cubic_function_4pts_jax(a1_new, a1s, y, int_a1), in_axes=0),
    #     in_axes=0
    # )(interp_a2)  # shape: (4,4)

    # # Step 4: Interpolate along mtot for all [i]
    # interp_mtot = vmap(lambda y: cubic_function_4pts_jax(mtot_new, ms, y, int_m), in_axes=0)(interp_a1)  # shape: (4,)

    # # Step 5: Interpolate along q (outermost axis)
    # final_snr = cubic_function_4pts_jax(q_new, qs, interp_mtot, int_q)  # shape: ()
    # return final_snr
...
def cubic_spline_2d_jax(q_array, mtot_array, snrpartialscaled_array, q_new, mtot_new):
    ...
    # return snr_final
    # Interpolate along the second axis (mtot) for each row of q

    # vectorized interpolation along mtot for each row of F
    interp_along_m = vmap(lambda f_row: cubic_function_4pts_jax(mtot_new, ms, f_row, int_m))(F)  # shape (4,)

    # Now, interpolate the resulting 4 values along the q axis
    snr_final = cubic_function_4pts_jax(q_new, qs, interp_along_m, int_q)
    return snr_final
