...
class TestBBHHorizonDistanceCalculation:
    ...
    def test_horizon_distance_bbh_numerical(self):
        ...
        horizon_distance_dict_validation, sky_location_dict_validation = (
            gwsnr.horizon_distance_numerical(mass_1=30.0, mass_2=30.0)
        )

        start = time.time()

        # horizon distance calculation for a BBH event (mass_1=mass_2=30Mo)
        horizon_distance_dict, sky_location_dict = gwsnr.horizon_distance_numerical(
        ...
        # print results
        print(
            f"\nHorizon distance (Mpc) for {gwsnr.detector_list} network: {horizon_distance_dict['horizon_distance_net']}"
        )
        print(
            f"Optimised sky location (ra, dec in rad) for {gwsnr.detector_list} network: {sky_location_dict['optimal_sky_location_net']}"
        )

        for key, value in horizon_distance_dict.items():
            name = (
                key
                if key != "horizon_distance_net"
                else f"{gwsnr.detector_list} network"
            )
            ...
            assert np.isreal(
                value
            ), f"horizon distance must be real (no complex numbers)"
            assert (value > 1000) and (
                value < 10000
            ), f"horizon distance for {name} network must be in a reasonable astrophysical range, got {value}"
        ...
        for key, value in sky_location_dict.items():
            name = (
                key
                if key != "optimal_sky_location_net"
                else f"{gwsnr.detector_list} network"
            )
            # ra check
            assert (value[0] >= 0.0) and (
                value[0] <= 2 * np.pi
            ), f"right ascension (ra) for {name} must be in [0, 2pi], got {value[0]}"
            # dec check
            assert (value[1] >= -np.pi / 2) and (
                value[1] <= np.pi / 2
            ), f"declination (dec) for {name} must be in [-pi/2, pi/2], got {value[1]}"
            # # validation for result consistency
        ...
        assert (
            execution_time < 60
        ), f"horizon distance calculation took too long: {execution_time} seconds"

        # array input should raise TypeError
        with pytest.raises(TypeError):
            horizon_distance_dict, sky_location_dict = gwsnr.horizon_distance_numerical(
                mass_1=np.array([30.0]), mass_2=np.array([30.0])
            )
    ...
    def test_horizon_distance_bbh_analytical(self):
        ...
        # validation run
        # this also helps initialize any internal njit functions
        horizon_distance_dict_validation = gwsnr.horizon_distance_analytical(
            mass_1=np.array([30.0]), mass_2=np.array([30.0])
        )
        ...
        # horizon distance calculation for a BBH event (mass_1=mass_2=30Mo)
        horizon_distance_dict = gwsnr.horizon_distance_analytical(
            mass_1=30.0, mass_2=30.0
        )
        ...
        for key, value in horizon_distance_dict.items():
            ...
            assert isinstance(
                value, np.ndarray
            ), f"expected np.ndarray, got {type(value)}"
            assert np.isfinite(
                value
            ).all(), f"horizon distance must be finite (no NaN/inf)"
            assert np.isreal(
                value
            ).all(), f"horizon distance must be real (no complex numbers)"
            assert (value > 1000).all() and (
                value < 10000
            ).all(), f"horizon distance for {key} detector must be in a reasonable astrophysical range, got {value}"
